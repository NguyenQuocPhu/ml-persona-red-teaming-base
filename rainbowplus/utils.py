...
def save_ga_iteration_log(
    ...
    Args:
        log_dir: Directory for saving log files
        ga_instance: The GrowingArchive object instance
        timestamp: Timestamp for log filename
        iteration: Current iteration number (-1 for global/final)
        max_iters: Maximum iterations configured
    """
    if iteration == -1:
    ...
    # Deepcopy to avoid modifying the live object during serialization
    clean_elites = {}
    for cell_id, elite_data in ga_instance.elites.items():
        # Convert each elite's data (especially 'behavior' vector) to list
        clean_item = elite_data
        
        # Optional: Remove 'behavior' from elite details if it makes the file too heavy
        # because centroids already store the location. 
        # But keeping it is safer for analysis.
        clean_elites[str(cell_id)] = clean_item

    # 2. Prepare Centroids Data
    # Only save the active centroids (up to n_centroids)
...
def save_ga_comprehensive_log(
    ...
    # 1. Extract History Archives (Same as original)
    history_data = {
        "max_iters": max_iters,
    ...
    # 2. Extract GA State (Centroids & Elites)
    active_centroids = ga_instance.centroids[:ga_instance.n_centroids]
    ...
    for cell_id, elite_data in ga_instance.elites.items():
        ga_state["elites"][str(cell_id)] = elite_data
    ...
    # 3. Combine Data
    full_log_data = {
        **history_data, # Unpack history keys
        "ga_state": ga_state # Add GA state under a specific key
    }

    # Calculate diversity metrics for GA elites
    try:
        # SỬA LỖI 3: Gọi hàm diversity dành cho LIST, không phải hàm dành cho ARCHIVE
